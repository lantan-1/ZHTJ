...
const orgType = computed(() => userStore.organizationType || '团支部');
const memberCount = computed(() => userStore.organizationMemberCount || 0);
const parentOrg = computed(() => ''); // 这里需要从API获取上级组织信息
  ...
  try {
    ...
    if (!userStore.organization || !userStore.name) {
      ...
      if (token) {
        ...
        console.log('团支部基本资料组件: 用户信息重新获取成功');
      } else {
        console.warn('团支部基本资料组件: 未找到有效令牌');
        ElMessage.warning('未找到登录凭证，请重新登录');
      }
    }
  } catch (error) {
    console.error('团支部基本资料组件: 获取用户信息失败', error);
...
<template>
  <el-card shadow="hover" class="info-card" v-loading="!orgName">
    <template #header>
    ...
    <el-descriptions :column="1" border v-if="orgName">
      ...
      <el-descriptions-item label="团员人数">{{ memberCount }}</el-descriptions-item>
      <el-descriptions-item label="上级组织">{{ parentOrg }}</el-descriptions-item>
    </el-descriptions>
      </el-tab-pane>
    </el-tabs>
